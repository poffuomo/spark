...
# Shell script to start a slave on the machine this script is executed on, even if a slave is
# already running on the same machine. This is useful e.g. for faking the dynamic allocation of
# executors in Spark local deployment mode.
...
# Usage: start-additional-slave.sh <SparkMasterURL>
#   Starts additional slaves on this worker machine, even if some slaves are already running in it
...
if [ "$SPARK_WORKER_INSTANCES" = "" ]; then
  start_instance $(( 1 + $EXISTING_WORKER_INSTANCES )) "$@"
else
  for ((i=0; i<$SPARK_WORKER_INSTANCES; i++)); do
    start_instance $(( 1 + $EXISTING_WORKER_INSTANCES + $i )) "$@"
  done
fi
